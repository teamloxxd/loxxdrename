...
# ─────────────── GPLinks Shortlink + Token ─────────────── #

import requests

def create_gplink(long_url: str) -> str:
    try:
        response = requests.post(
            "https://gplinks.in/api",
            params={"api": GP_API_KEY, "url": long_url},
            timeout=10
        )
        data = response.json()
        return data.get("shortenedUrl", long_url)
    except Exception as e:
        print(f"[GPLINK ERROR] {e}")
        return long_url

@Client.on_message(filters.command("gettoken") & filters.private)
async def get_token(client, message):
    user = message.from_user
    original_url = "https://t.me/Madflix_Bots"  # Update to your monetized URL
    short_url = create_gplink(original_url)

    await message.reply(
        text="🔐 **4GB Token Access**\nClick the button below, complete the steps, then come back and press 'I Opened Link'.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔗 Get Token", url=short_url)],
            [InlineKeyboardButton("✅ I Opened Link", callback_data="activate_token")]
        ])
    )
